...
in {
  ...
  # ── 1. NAS auto-mount ────────────────────────────────────────────────
  # Runs at login and every 5 minutes; ensures the SMB share is mounted.
  # If already mounted, exits immediately (< 1 ms).
  ...
  # Watches the exports directory and runs the converter every 10 minutes,
  # plus on any file change via WatchPaths.
  launchd.user.agents.opencode-session-sync = {
    serviceConfig = {
      ...
      # Watch the session export directory for changes
      WatchPaths = [
        sessionSrc
      ];

      # Also run on a schedule every 10 minutes as a safety net
      StartCalendarInterval = [
        {Minute = 0;}
        {Minute = 10;}
        {Minute = 20;}
      ...
      # Throttle: don't fire more than once per 30 seconds
      ThrottleInterval = 30;
